...
class block2HybridBlock(nn.HybridBlock):
    def __init__(self, **kwargs):
        ...
        # self.stacked_rnn_cells.add(rnn.DropoutCell(0.5))

    def hybrid_forward(self, F, x):
        enout, (states) = self.stacked_rnn_cells.unroll(inputs=x, length=5, merge_outputs=True)
        return (enout)
    ...
    def __init__(self, **kwargs):
        ...
        with self.name_scope():
            ...
            with layer3.name_scope():
                layer3.add(nn.BatchNorm(axis=2))
                layer3.add(nn.Dense(1, flatten=True))
            # chain blocks together
            self.net = nn.HybridSequential()
            self.net.add(layer2, layer3)
